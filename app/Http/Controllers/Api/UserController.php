...
class UserController extends ApiBaseController {
    ...
    public function sendMessage(Request $request) {
        ...
        $messages_from_data = Messages::where('from_user_id', $user->id)
                                    ->where('to_user_id', $request->receiver_id)
                                    ->first();
        ...
        if($messages_from_data){
            ...
            $messages_to_data = Messages::where('from_user_id', $request->receiver_id)
                                    ->where('to_user_id', $user->id)
                                    ->first();
        ...
        // Create new room if no conversation exists
        if($room == 'false'){
            $messages_room = new MessagesRoom;
            $messages_room->md5_id = '';
            $messages_room->creator_id = $user->id;
            $messages_room->save();
            
            // Update with md5 id
            $messages_room = MessagesRoom::find($messages_room->id);
            $messages_room->md5_id = md5($messages_room->id);
            $messages_room->save();
            $room_id = $messages_room->id;
        }
        
        // Create message - add content to request for Messages model
        $request->merge(['content' => $request->message]);
        $message_data = Messages::addData($room_id, $user->id, $request->receiver_id, $request);
